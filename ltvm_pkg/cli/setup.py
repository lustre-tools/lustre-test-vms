...
def cmd_doctor(args: argparse.Namespace) -> int:
    use_json = args.json
    # Every check is a read.  Only a repair can need root, and a user
    # without sudo still gets the report.
    if args.fix and not use_json:
        from ltvm_pkg.priv import SudoUnavailable, sudo_prime

        try:
            sudo_prime("ltvm doctor --fix needs root for some repairs")
        except SudoUnavailable as e:
            log.warning("%s -- repairs that need root will fail", e)
...
def _repo_owner(repo: Path) -> tuple[int, int, str] | None:
    """(uid, gid, home) of the human who owns ``repo``, or None for us.

    ``ltvm update`` is root-gated, but a ``git pull`` run as root
    rewrites every changed file as root in a checkout that belongs to a
    human -- who then cannot edit the tree, and whose next `git pull`
    fails on the root-owned objects.  Returns None when we are not root
    or the checkout really is root's, in which case git runs as us.
    """
    if os.geteuid() != 0:
        return None
    try:
    ...
    try:
        home = pwd.getpwuid(st.st_uid).pw_dir
    except KeyError:
        return None
    return st.st_uid, st.st_gid, home
...
def cmd_update(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if new_hash:
            build_info = repo / "ltvm_pkg" / "_build_info.py"
            ...
            owner = _repo_owner(repo)
            if owner is not None:
                os.chown(build_info, owner[0], owner[1])
