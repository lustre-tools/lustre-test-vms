...
check(
    f"base.ext4 built rc={r.returncode} {r.stderr.strip()[:60]}",
    r.returncode == 0,
)
...
class Shim:
    ...
    def resolve_kernel(self, k=None):
        return "k1"

    def image_output_dir(self, k=None):
        return imgdir

    def kernel_output_dir(self, k=None):
        return kdir
...
result = ie.export_image(
    Shim(), None, out, image_format="gce", ssh_key=sshkey, force=True
)
...
check(
    f"exactly one member, named disk.raw: {[m.name for m in members]}",
    [m.name for m in members] == ["disk.raw"],
)
raw_size = members[0].size
GIB = 1024**3
check(
    f"disk.raw is a whole number of GiB ({raw_size / GIB:.2f} GiB)",
    raw_size % GIB == 0,
)
check(
    f"tarball stayed small via sparse packing "
...
check(
    "msdos label with one bootable partition",
...
try:
    ...
    check(
        "fstab / entry rewritten to the real filesystem UUID",
        f"UUID={uuid}" in fstab,
    )
    check(
        "no /dev/vda left on the / line",
        not any(
            line.split()[:2] == ["/dev/vda", "/"]
    ...
    check(
        "profile is DHCP on eth0",
        "method=auto" in nm.read_text()
        and "interface-name=eth0" in nm.read_text(),
    )
    check(
        f"profile mode is 0600 (NM ignores it otherwise): "
    ...
    check(
        f"symlink points at the unit: {os.readlink(want)}",
...
if r.returncode == 0:
    ...
    check(
        f"qemu-img says qcow2 ({info.get('format')})",
